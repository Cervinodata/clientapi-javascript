...
> String getGA4ReportPerDevicePerChannelGroupPerOrganisationPerPropertyPerDay(organisationUuids, opts)
...
apiInstance.getGA4ReportPerDevicePerChannelGroupPerOrganisationPerPropertyPerDay(organisationUuids, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
